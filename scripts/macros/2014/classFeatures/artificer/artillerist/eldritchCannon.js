...
import {activityUtils, actorUtils, compendiumUtils, constants, dialogUtils, effectUtils, genericUtils, itemUtils, rollUtils, tokenUtils, workflowUtils} from '../../../../../utils.js';
...
async function use({workflow}) {
    ...
    async function cannonUpdates() {
        ...
        let damageBonus = classLevel >= 9 ? '1d8' : undefined;
        if (cannonType === 'Protector') {
            let intBonus = Math.max(1, workflow.actor.system.abilities.int.mod);
            if (damageBonus) {
    ...
    let commandFeature = activityUtils.getActivityByIdentifier(workflow.item, 'eldritchCannonCommand', {strict: true});
    ...
    await Summons.spawn(sourceActors, updates, workflow.item, workflow.token, {
        ...
        additionalVaeButtons: [{type: 'use', name: commandFeature.name, identifier: 'eldritchCannon', activityIdentifier: 'eldritchCannonCommand'}],
        unhideActivities: {
            itemUuid: workflow.item.uuid,
            activityIdentifiers: ['eldritchCannonCommand', 'eldritchCannonDismiss'],
            favorite: true
        }
    });
    if (costsSlotType) {
        ...
        item: [
            {
                ...
                macro: use,
                priority: 50,
                activities: ['eldritchCannon']
            },
            {
                pass: 'rollFinished',
                macro: dismiss,
                priority: 50,
                activities: ['eldritchCannonDismiss']
            },
            {
                pass: 'preTargeting',
                macro: early,
