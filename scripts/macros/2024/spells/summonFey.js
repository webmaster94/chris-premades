...
import {activityUtils, actorUtils, compendiumUtils, constants, dialogUtils, effectUtils, errors, genericUtils, itemUtils, templateUtils, tokenUtils, workflowUtils} from '../../../utils.js';
async function use({workflow}) {
    let activityIdentifier = activityUtils.getIdentifier(workflow.activity);
    let concentrationEffect = effectUtils.getConcentrationEffect(workflow.actor, workflow.item);
    let sourceActor = await compendiumUtils.getActorFromCompendium(constants.modernPacks.summons, 'CPR - Fey Spirit');
    if (!sourceActor) {
    ...
    if (activityIdentifier === 'summonFeyFuming') {
        creatureType = 'fuming';
    } else if (activityIdentifier === 'summonFeyMirthful') {
        creatureType = 'mirthful';
    } else if (activityIdentifier === 'summonFeyTricksy') {
        creatureType = 'tricksy';
    }
    if (!creatureType) {
        if (concentrationEffect) await genericUtils.remove(concentrationEffect);
        return;
    ...
    let multiAttackFeatureData = await Summons.getSummonItem('Multiattack (Fey Spirit)', {}, workflow.item, {translate: genericUtils.format('CHRISPREMADES.CommonFeatures.Multiattack', {numAttacks}), identifier: 'summonFeyMultiattack', rules: 'modern'});
    let feyBladeFeatureData = await Summons.getSummonItem('Fey Blade (Fey Spirit)', {}, workflow.item, {translate: 'CHRISPREMADES.Macros.SummonFey.FeyBlade', identifier: 'summonFeyFeyBlade', flatAttack: true, damageBonus: spellLevel, rules: 'modern'});
    ...
    }, workflow.item, {translate: 'CHRISPREMADES.Macros.SummonFey.FeyStep', identifier: 'summonFeyFeyStep', rules: 'modern'});
    if (!multiAttackFeatureData || !feyBladeFeatureData || !feyStepFeatureData) {
    ...
    let updates = {
        actor: {
            ...
            items: [multiAttackFeatureData, feyBladeFeatureData, feyStepFeatureData]
        },
        token: {
            name,
            disposition: workflow.token.document.disposition
    ...
    let moodFeatureData = await Summons.getSummonItem(creatureType.capitalize(), {}, workflow.item, {translate: 'CHRISPREMADES.Macros.SummonFey.' + creatureType.capitalize(), flatDC: creatureType === 'mirthful', identifier: 'summonFey' + creatureType.capitalize(), rules: 'modern'});
    if (!moodFeatureData) {
        errors.missingPackItem();
        if (concentrationEffect) await genericUtils.remove(concentrationEffect);
        return;
    ...
    await Summons.spawn(sourceActor, updates, workflow.item, workflow.token, {
        ...
        additionalSummonVaeButtons: [multiAttackFeatureData, feyBladeFeatureData, feyStepFeatureData].map(i => {return {type: 'use', name: i.name, identifier: i.flags['chris-premades'].info.identifier};})
    });
}
async function late({workflow}) {
    ...
    if (fumingItem) {
        ...
        if (effect) await genericUtils.update(effect, {'duration.value': 1, 'duration.units': 'turns', start: {round: game.combat?.round ?? 0, turn: game.combat?.turn ?? 0}});
        if (itemUtils.getConfig(workflow.item, 'useRealDarkness')) {
            let radius = templateUtils.getRadius(template);
            let center = templateUtils.getPosition(template);
            let darknessSourceArr = await genericUtils.createEmbeddedDocuments((templateUtils.getRegionDoc(template) ?? template).parent, 'AmbientLight', [{config: {negative: true, dim: templateUtils.getDistance(template) / 2, animation: {type: itemUtils.getConfig(workflow.item, 'darknessAnimation')}}, x: center.x + radius, y: center.y + radius}]);
