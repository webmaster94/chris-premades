...
async function use({workflow}) {
    ...
    if (flavor === 'offTarget') {
        ...
        ChatMessage.create({
            speaker: workflow.chatCard.speaker,
            content: genericUtils.format('CHRISPREMADES.Macros.Teleport.OffTargetString', {distance, units: genericUtils.translate('CHRISPREMADES.Units.Miles').toLowerCase(), direction: directions[roll2.total - 1]}),
            whisper: [socketUtils.gmID()],
            blind: true
        });
    }
